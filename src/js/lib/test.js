...
function init() {
    ...
    (camera = new THREE.PerspectiveCamera(30, e, 1, 1e3)).position.z = 400, (renderer = new THREE.WebGLRenderer({
        antialias: !0
    })).setSize(window.innerWidth, window.innerHeight), renderer.setPixelRatio(window.devicePixelRatio), renderer.setClearColor(2106669), renderer.shadowMapEnabled = !0, renderer.shadowMapSoft = !0, document.body.appendChild(renderer.domElement);
    var t = new THREE.SpotLight(0, .75);
    t.position.set(-10, 80, 20), t.target.position.set(-200, 88, 10), t.castShadow = !0, t.shadowDarkness = .2, t.shadowBias = 1e-5, helper = new THREE.SpotLightHelper(t), t.shadowMapWidth = 2048, t.shadowMapHeight = 2048;
    var n = new THREE.SpotLight(0, .95);
    n.position.set(40, 80, 30), n.target.position.set(0, -20, -30), n.castShadow = !0, n.shadowDarkness = .2, t.shadowBias = .001;
    ...
    a.castShadow = !0, a.shadowDarkness = .2, a.shadowMapWidth = 2048, a.shadowMapHeight = 2048;
    new THREE.DirectionalLightHelper(a, 1);
    var o = new THREE.PointLight(10824509, .7, 100);
    o.position.set(110, -5, 10), o.shadowDarkness = .4, o.shadowColor = 8018261;
    new THREE.PointLightHelper(o);
    var i = new THREE.PointLight(10824509, .7, 100);
    i.position.set(0, 30, 10);
    new THREE.PointLightHelper(i);
    var r = new THREE.AmbientLight(7364704, 3);
    r.shadowDarkness = .08, r.shadowColor = 3656831;
    ...
    var w = new THREE.LineDashedMaterial({
            color: 10902632,
    ...
    g.position.set(131, -14, 0), g.rotation.x = Math.PI / 9, g.rotation.y = -Math.PI / 4, g.scale.set(1.65, 1.75, 1.5), scene.add(a), scene.add(n), scene.add(o), scene.add(i), scene.add(r), scene.add(d), scene.add(l), scene.add(h), scene.add(y), scene.add(cylinder1), scene.add(cylinder2), scene.add(cylinder3), scene.add(cylinder4), scene.add(m), scene.add(M), scene.add(E), scene.add(g), objectControls = new ObjectControls(camera);
    ...
    for (var L = 0; L < mesh.length; L++) mesh[L].scale.set(.26, .26, .26), mesh[L].castShadow = !0, mesh[L].receiveShadow = !0, mesh[L].selected = !1, mesh[L].select = function() {
    ...
    }, scene.add(mesh[L]), objectControls.add(mesh[L]);
    window.addEventListener("resize", onWindowResize, !1), console.log("Check Out My Works Here:" + "%c www.yanlinma.com ", "background: #355493; color: #fff"), window.addEventListener("load", function() {
...
function generateSound() {
    ...
    for (var o = cylinder1.position.clone(), i = 0; i < cylinder1.geometry.vertices.length; i++) {
        ...
        if (s.length > 0 && s[0].distance < r.length()) {
            switch (e.innerHTML = "contact1:  " + s[0].distance, plane2.material = new THREE.MeshLambertMaterial({
                color: 13837885
            }), Math.floor(s[0].distance)) {
                case 7:
                    note2 = "E4";
                    break;
                case 8:
    ...
    for (var c = cylinder2.position.clone(), d = 0; d < cylinder2.geometry.vertices.length; d++) {
        ...
        if (h.length > 0 && h[0].distance < l.length()) {
            switch (t.innerHTML = "contact2:  " + h[0].distance, Math.floor(h[0].distance)) {
                case 5:
                case 6:
                case 7:
                    note1 = "C6";
    ...
    for (var y = cylinder3.position.clone(), w = 0; w < cylinder2.geometry.vertices.length; w++) {
        ...
        if (m.length > 0 && m[0].distance < p.length()) {
            switch (n.innerHTML = "contact3:  " + m[0].distance, Math.floor(m[0].distance)) {
                case 5:
                case 6:
                case 7:
                    note3 = "C3";
    ...
    for (var M = cylinder4.position.clone(), E = 0; E < cylinder4.geometry.vertices.length; E++) {
        ...
        if (v.length > 0 && v[0].distance > 4 && v[0].distance < g.length()) {
            switch (a.innerHTML = "contact4:  " + v[0].distance, Math.floor(v[0].distance)) {
                case 5:
                    note4 = "C3";
                    break;
                case 6:
...
function changeGeoColor() {
    for (var e = 0; e < mesh.length; e++) Math.abs(mesh[e].position.y - cylinder1.position.y) < 80 && Math.abs(mesh[e].position.x - cylinder1.position.x) < 30 || Math.abs(mesh[e].position.y - cylinder2.position.y) < 80 && Math.abs(mesh[e].position.x - cylinder2.position.x) < 30 || Math.abs(mesh[e].position.y - cylinder3.position.y) < 80 && Math.abs(mesh[e].position.x - cylinder3.position.x) < 30 || Math.abs(mesh[e].position.y - cylinder4.position.y) < 180 && Math.abs(mesh[e].position.x - cylinder4.position.x) < 30 ? mesh[e].material = new THREE.MeshLambertMaterial({
        color: 13004569
    }) : mesh[e].material = new THREE.MeshLambertMaterial({
        color: 10389124
    })
}
var scene, camera, renderer, objectControls, objects = [],
    mesh = [],
    speed = [],
    particlesPool = [],
    counter = 0;
...
var intersectionPlane, collidableMeshList = [],
...
var CylinderGeo = new THREE.CylinderGeometry(.15, .15, 25),
